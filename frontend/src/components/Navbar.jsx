...
export default function Navbar() {
  ...
  const linkCls = ({ isActive }) =>
    `px-3 py-2 rounded-md transition-colors hover:text-brand-700 hover:bg-brand-50 dark:hover:bg-slate-800 ${
      isActive 
        ? 'text-brand-700 bg-brand-50 dark:text-blue-300 dark:bg-slate-800' 
      ...
      <div className="mx-auto max-w-7xl px-4 py-3">
        <div className="flex items-center gap-4">
          ...
          <div className="hidden md:flex items-center gap-1">
            ...
            {currentUser && <NavLink className={linkCls} to="/dashboard">Dashboard</NavLink>}
            {currentUser && <NavLink className={linkCls} to="/daily-checkin">Daily Check-in</NavLink>}
            {currentUser && <NavLink className={linkCls} to="/upload-report">Upload Report</NavLink>}
          </div>
          <div className="flex items-center gap-2 relative">
            <button
              ...
              className="relative inline-flex w-14 h-8 rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-brand-600 focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-slate-900 bg-blue-50 dark:bg-slate-700"
            >
                ...
                >
                  <span className="relative inline-block">
                    <span className="w-8 h-8 rounded-full grid place-items-center text-white text-sm font-medium bg-gradient-to-tr from-brand-600 to-blue-500 ring-2 ring-brand-500/40">
                      {currentUser.email ? currentUser.email.slice(0, 1).toUpperCase() : '?'}
                    </span>
            ...
            <button 
              className="md:hidden inline-flex items-center justify-center w-9 h-9 rounded-md border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-200" 
              onClick={() => setOpen(o => !o)} 
              aria-expanded={open} 
        ...
        {open && (
          <div id="mobile-nav" className="md:hidden mt-3 grid gap-1">
            ...
            {currentUser && <NavLink onClick={() => setOpen(false)} className={linkCls} to="/dashboard">Dashboard</NavLink>}
            {currentUser && <NavLink onClick={() => setOpen(false)} className={linkCls} to="/daily-checkin">Daily Check-in</NavLink>}
            {currentUser && <NavLink onClick={() => setOpen(false)} className={linkCls} to="/upload-report">Upload Report</NavLink>}
            {currentUser ? (
              <button
                onClick={handleLogout}
